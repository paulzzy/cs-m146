...
def plot_gallery(images, title="plot", subtitles=[], h=50, w=37, n_row=3, n_col=4):
    """
...
def PCA(X):
    ...
    U = np.array(
        [vi / 1.0 for (li, vi) in sorted(zip(l, v), reverse=True, key=lambda x: x[0])]
    ).T
    return U, mu
